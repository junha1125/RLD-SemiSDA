...
def get_config():
    ...
    parser.add_argument('--use_incl', action='store_true', help='Use incl to plot and save curves')
    ...
    parser.add_argument('--num_save_epoch', type=int, default=0,
                        help='model saving frequency. if 0, only save the last and best model')
    parser.add_argument('--partial_ratio', type=float, default=1)
    ...
    parser.add_argument('--net_from_torch', type=str2bool, default=False)
    parser.add_argument('--use_pretrain', default=False, type=str2bool, help="if False, train from scratch")      
    parser.add_argument('--pretrain_path', default='', type=str,
                        help="ResNet_from_torch use ImageNet pretrained model, while the paths for ViT have already discribed in USB config file")  
    parser.add_argument('--linear_probing', type=str2bool, default=False)
    # parser.add_argument('--bottleneck_dim', type=int, default=256,
    #                     help='if > 0, use additional BN1d after feature encoding.')
    # parser.add_argument('--weight_norm_dim', type=int, default=0,
    #                     help='if >= 0, use weight norm on the last fc layer of the backbone')
    ...
    parser.add_argument('--trg_eval_src', type=str2bool, default=False, help='during finetuing, evaluate on source domain')
    parser.add_argument('--negatively_biased_feedback', type=str2bool, default=False, help='only samples inferred as false prediction are included in labeled data sets')
    parser.add_argument('--save_feature', type=str2bool, default=False, help='save features after each epoch')
...
def main(args):
    ...
    if args.resume:
        if args.load_path is None:
            args.load_path = os.path.join(save_path, "latest_model.pth")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    _net_builder = get_net_builder(args.net, args.net_from_torch)
    ...
    model.model = send_model_cuda(args, model.model, clip_batch=False)
    model.ema_model = send_model_cuda(args, model.ema_model, clip_batch=False)
    logger.info(f"Arguments: {model.args}")

    # Load and customize the source model.
    if args.trg: 
        if os.path.exists(args.src_model_path):
            model.load_model(args.src_model_path)
        else:
            raise Exception(f"Source model path {args.src_model_path} does not exist")
