...
def get_visda(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    if args.src:
        # Split datas and targets into train and validation data.
        datas, targets = np.array(datas), np.array(targets)
        total_data_size, train_ratio = len(datas), 0.9 # VisDA has 10% validation data.
        ...
        if args.trg_eval_mode == 'finetune=valset':
            ...
            datas, targets = np.array(datas), np.array(targets)
            train_idx, val_idx = sample_train_val_data(args, datas, targets, num_classes, train_ratio=0.9)
            train_datas = datas[train_idx]
            train_targets = targets[train_idx]
            val_datas =  datas[val_idx]
            val_targets = targets[val_idx]
    ...
    elif args.trg_eval_src == True or args.trg == 'train':
        # TODO: Remove redundancy code.
        if args.trg_eval_mode == 'finetune=valset':
            val_datas, val_targets = datas, targets
        elif args.trg_eval_mode == 'finetune!=valset':
            datas, targets = np.array(datas), np.array(targets)
            _, val_idx = sample_train_val_data(args, datas, targets, num_classes, train_ratio=0.9)
            val_datas =  datas[val_idx]
            val_targets = targets[val_idx]
        else:
            raise ValueError(f"Invalid trg_eval_mode: {args.trg_eval_mode}")
        lb_dset, ulb_dset = None, None
        eval_dset = BasicDatasetFromPath(alg, val_datas, val_targets, num_classes, transform_val, False, None, False)
