...
def get_officehome(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    if args.src:
        ...
        total_data_size, train_ratio = len(datas), 0.8 # OfficeHome has 20% validation data.
        ...
        if args.trg_eval_mode == 'finetune=valset':
            ...
            datas, targets = np.array(datas), np.array(targets)
            train_idx, val_idx = sample_train_val_data(args, datas, targets, num_classes, train_ratio=0.8, file_name=f"oh_{domain_name}_valset.npy")
            train_datas = datas[train_idx]
            train_targets = targets[train_idx]
            val_datas =  datas[val_idx]
            val_targets = targets[val_idx]
