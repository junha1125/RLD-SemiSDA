...
class AdaMoCo(nn.Module):
    ...
    def __init__(
        ...
        # create the fc heads
        if hasattr(src_model.module, "output_dim"):
            feature_dim = src_model.module.output_dim
        elif hasattr(src_model.module, "embed_dim"):
            feature_dim = src_model.module.embed_dim
        else:
        ...
        self.register_buffer(
            "mem_labels", torch.randint(0, src_model.module.num_classes, (K,))
        )
        self.mem_feat = F.normalize(self.mem_feat, dim=0).cuda(gpu)
        self.mem_labels = self.mem_labels.cuda(gpu)
    ...
    def forward(self, im_q, im_k=None, cls_only=False):
        ...
        outputs = self.src_model(im_q)
        logits_q = outputs['logits']
        feats_q = outputs['feat']
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            outputs_k = self.momentum_model(im_k)
            k = outputs_k['feat']
