...
def load_checkpoint(model, checkpoint_path):
    ...
    print("[Load_imageNet_model]"+str(match))
    return model
...
def param_groups(net: nn.Module, lr: float):
    if not hasattr(net, 'get_params'): raise Exception(f'get_params is not defined in net: {net}')
    backbone_params, classifier_params = net.get_params()
    return [
        {'params': classifier_params, 'lr': lr*10, 'name': 'classifier'},
