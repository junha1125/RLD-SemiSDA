...
from semilearn.nets.utils import param_groups, param_groups_layer_decay, param_groups_weight_decay

def get_net_builder(net_name, net_from_torch: bool):
    """
    ...
    if net_from_torch:
        ...
        if net_name not in model_name_list:
            ...
            return torch_net_builder(net_name)
    else:
        # TODO: fix bug here + if net_name is not nets.__dict__, then raise error.
        import semilearn.nets as nets
        builder = getattr(nets, net_name)
        return builder

def torch_net_builder(net_name):
    """
    built network according to network name from torchvision_models
    """
    import semilearn.nets as nets
    if net_name == 'resnet34':
        builder = getattr(nets, 'resnet34_torch')
    elif net_name == 'resnet50':
        builder = getattr(nets, 'resnet50_torch')
    elif net_name == 'resnet101':
        builder = getattr(nets, 'resnet101_torch')
    else:
        raise NotImplementedError(f'net_name {net_name} is not implemented.')
    return builder
...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    from semilearn.datasets import get_visda, get_domainnet, get_officehome
    if dataset == "eurosat":
    ...
    # Customization dataset
        ...
        lb_dset, ulb_dset, eval_dset = get_visda(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset == 'domainnet':
        lb_dset, ulb_dset, eval_dset = get_domainnet(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    elif dataset == 'officehome':
        lb_dset, ulb_dset, eval_dset = get_officehome(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    # speech dataset
...
def get_optimizer(net, optim_name='SGD', lr=0.1, momentum=0.9, weight_decay=0, layer_decay=1.0, net_name=False, linear_probing=False, nesterov=True, bn_wd_skip=True):
    '''
    ...
    '''
    if net_name in ['vit_clip', 'resnet50', 'resnet101']:
        # TODO: Note that  ars.net_from_torch == False is not supported when resnet50 and resnet101 are used.
        per_param_args = param_groups(net, lr)
        if linear_probing:
            for param in net.encoder.parameters():
                param.requires_grad = False
            per_param_args = [param for param in per_param_args if param['name'] == 'classifier']
    else:
        ...
        if layer_decay != 1.0:
            ...
            per_param_args = param_groups_weight_decay(net, lr, weight_decay, no_weight_decay_list=no_decay)
