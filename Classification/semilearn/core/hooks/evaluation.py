...
class EvaluationHook(Hook):
    ...
    def before_run(self, algorithm):
        if algorithm.args.trg:
            algorithm.print_fn("validating...")
            # algorithm.print_fn("[EvaluationHook] WARNING, uncomment the following lines to evaluate on target domain during training")
            eval_dict = algorithm.evaluate('eval')
            algorithm.log_dict.update(eval_dict)
            if algorithm.args.trg_eval_src:
                eval_dict = algorithm.evaluate('eval_src')
                algorithm.log_dict.update(eval_dict)
